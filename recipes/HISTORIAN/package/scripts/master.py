...
class DemoControl(Script):
  def install(self, env):
    ...
    Execute(params.install_dir+'/CloudBreakArtifacts/recipes/historian-service-install.sh '+params.historian_dir)

  def start(self, env):
    self.configure(env)
    import params
    Execute('echo Start UI')
    Execute('docker run -d -p '+params.ui_port+':8080 -e ATLAS_HOST='+params.atlas_host_ip+' -e ATLAS_PORT='+params.atlas_port+' -e API_HOST='+params.nifi_host_ip+' -e API_PORT='+params.api_port+' -t hortonworks/historian')
