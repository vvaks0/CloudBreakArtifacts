...
deployTemplateToNifi () {
    ...
    TEMPLATEID=$(curl -v -F template=@"$ROOT_PATH/Twitter_Dashboard.xml" -X POST http://$AMBARI_HOST:9090/nifi-api/process-groups/root/templates/upload | grep -Po '<id>([a-z0-9-]+)' | grep -Po '>([a-z0-9-]+)' | grep -Po '([a-z0-9-]+)')
        sleep 60
